...
namespace DiscordBot6.Commands {
    [Group("channelmute")]
    public sealed class ChannelMutes : ModuleBase<SocketGuildCommandContext> {
        private static readonly TimeSpan minimumMuteTimeSpan = TimeSpan.FromMinutes(2);
        ...
        [Command("add")]
        public async Task AddCommand(ulong id, params string[] timespan) {
            await AddCommandImpl(id, timespan);
        }

        [Command("add")]
        public async Task AddCommand(IUser user, params string[] timespan) {
            await AddCommandImpl(user.Id, timespan);
        }
        ...
        private async Task AddCommandImpl(ulong userId, string[] timespan) {
            ...
            if (!await server.UserMayTempMute(Context.User.Id, userRoleIds)) {
            ...
            bool parsedTimeSpan = CommandHelper.GetTimeSpan(timespan, out TimeSpan timeSpan, out string[] errors, minimumMuteTimeSpan);
            if (!parsedTimeSpan) {
                await Context.Channel.CreateResponse()
                    .WithSubject(Context.User.Id, Context.User.GetAvatarUrl(size: 64))
        ...
        private async Task CheckCommandImpl(ulong userId) {
            ...
            IEnumerable<ulong> userRoleIds = Context.User.Roles.Select(role => role.Id);
            if (!await server.UserMayCheckMutePersists(Context.User.Id, userRoleIds)) {
                await Context.Channel.CreateResponse()
                        .WithSubject(Context.User.Id, Context.User.GetAvatarUrl(size: 64))
                        .SendNoPermissionAsync();
